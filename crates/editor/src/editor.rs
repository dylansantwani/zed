...
pub struct Editor {
    ...
    editor_actions: Rc<RefCell<BTreeMap<EditorActionId, Box<dyn Fn(&mut ModelContext<Self>)>>>>,
    use_autoclose: bool,
...
impl CodeActionsMenu {
    fn select_first(&mut self, cx: &mut ModelContext<Editor>) {
        self.selected_item = 0;
        self.scroll_handle
            .scroll_to_item(self.selected_item, ScrollStrategy::Top);
        cx.notify()
    ...
    fn select_prev(&mut self, cx: &mut ModelContext<Editor>) {
        if self.selected_item > 0 {
            self.selected_item -= 1;
        } else {
...
impl Editor {
    pub fn single_line(cx: &mut ModelContext<Self>) -> Self {
    ...
    pub fn multi_line(cx: &mut ModelContext<Self>) -> Self {
        let buffer = cx.new_model(|cx| Buffer::local("", cx));
        let buffer = cx.new_model(|cx| MultiBuffer::singleton(buffer, cx));
        Self::new(EditorMode::Full, buffer, None, false, cx)
    }

    pub fn auto_width(cx: &mut ModelContext<Self>) -> Self {
        let buffer = cx.new_model(|cx| Buffer::local("", cx));
        let buffer = cx.new_model(|cx| MultiBuffer::singleton(buffer, cx));
        Self::new(
    ...
    pub fn auto_height(max_lines: usize, cx: &mut ModelContext<Self>) -> Self {
        let buffer = cx.new_model(|cx| Buffer::local("", cx));
        let buffer = cx.new_model(|cx| MultiBuffer::singleton(buffer, cx));
        Self::new(
    ...
    pub fn clone(&self, cx: &mut ModelContext<Self>) -> Self {
    ...
    fn key_context(&self, cx: &ModelContext<Self>) -> KeyContext {
    ...
    pub fn set_custom_context_menu(
        &mut self,
        f: impl 'static
            + Fn(&mut Self, DisplayPoint, &mut ModelContext<Self>) -> Option<Model<ui::ContextMenu>>,
    ) {
    ...
    pub fn set_cursor_shape(&mut self, cursor_shape: CursorShape, cx: &mut ModelContext<Self>) {
    ...
    pub fn set_clip_at_line_ends(&mut self, clip: bool, cx: &mut ModelContext<Self>) {
    ...
    fn select(&mut self, phase: SelectPhase, cx: &mut ModelContext<Self>) {
        self.hide_context_menu(cx);
    ...
    fn end_selection(&mut self, cx: &mut ModelContext<Self>) {
    ...
    pub fn cancel(&mut self, _: &Cancel, cx: &mut ModelContext<Self>) {
    ...
    pub fn handle_input(&mut self, text: &str, cx: &mut ModelContext<Self>) {
    ...
    pub fn newline(&mut self, _: &Newline, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
    ...
    pub fn newline_above(&mut self, _: &NewlineAbove, cx: &mut ModelContext<Self>) {
    ...
    pub fn newline_below(&mut self, _: &NewlineBelow, cx: &mut ModelContext<Self>) {
        let buffer = self.buffer.read(cx);
        let snapshot = buffer.snapshot(cx);
    ...
    pub fn insert(&mut self, text: &str, cx: &mut ModelContext<Self>) {
    ...
    fn select_autoclose_pair(&mut self, cx: &mut ModelContext<Self>) {
        let selections = self.selections.all::<usize>(cx);
    ...
    pub fn toggle_inlay_hints(&mut self, _: &ToggleInlayHints, cx: &mut ModelContext<Self>) {
    ...
    fn refresh_inlay_hints(&mut self, reason: InlayHintRefreshReason, cx: &mut ModelContext<Self>) {
    ...
    fn visible_inlay_hints(&self, cx: &ModelContext<'_, Editor>) -> Vec<Inlay> {
        self.display_map
            .read(cx)
            .current_inlays()
    ...
    pub fn show_completions(&mut self, options: &ShowCompletions, cx: &mut ModelContext<Self>) {
        if self.pending_rename.is_some() {
            return;
        }
    ...
    pub fn toggle_code_actions(&mut self, action: &ToggleCodeActions, cx: &mut ModelContext<Self>) {
        let mut context_menu = self.context_menu.write();
    ...
    pub async fn open_project_transaction(
        ...
        workspace.update(&mut cx, |workspace, cx| {
            let project = workspace.project().clone();
            let editor =
                cx.new_model(|cx| Editor::for_multibuffer(excerpt_buffer, Some(project), true, cx));
    ...
    fn refresh_code_actions(&mut self, cx: &mut ModelContext<Self>) -> Option<()> {
        let buffer = self.buffer.read(cx);
        let newest_selection = self.selections.newest_anchor().clone();
    ...
    fn start_inline_blame_timer(&mut self, cx: &mut ModelContext<Self>) {
    ...
    fn refresh_document_highlights(&mut self, cx: &mut ModelContext<Self>) -> Option<()> {
        if self.pending_rename.is_some() {
            return None;
        }
    ...
    pub fn display_cursor_names(&mut self, _: &DisplayCursorNames, cx: &mut ModelContext<Self>) {
        self.show_cursor_names(cx);
    }

    fn show_cursor_names(&mut self, cx: &mut ModelContext<Self>) {
    ...
    fn update_visible_inline_completion(&mut self, cx: &mut ModelContext<Self>) {
    ...
    pub fn spawn_nearest_task(&mut self, action: &SpawnNearestTask, cx: &mut ModelContext<Self>) {
    ...
    fn hide_context_menu(&mut self, cx: &mut ModelContext<Self>) -> Option<ContextMenu> {
        cx.notify();
    ...
    pub fn move_to_next_snippet_tabstop(&mut self, cx: &mut ModelContext<Self>) -> bool {
        self.move_to_snippet_tabstop(Bias::Right, cx)
    }

    pub fn move_to_prev_snippet_tabstop(&mut self, cx: &mut ModelContext<Self>) -> bool {
        self.move_to_snippet_tabstop(Bias::Left, cx)
    }

    pub fn move_to_snippet_tabstop(&mut self, bias: Bias, cx: &mut ModelContext<Self>) -> bool {
    ...
    pub fn backspace(&mut self, _: &Backspace, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
    ...
    pub fn delete(&mut self, _: &Delete, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
            this.change_selections(Some(Autoscroll::fit()), cx, |s| {
    ...
    pub fn tab_prev(&mut self, _: &TabPrev, cx: &mut ModelContext<Self>) {
    ...
    pub fn tab(&mut self, _: &Tab, cx: &mut ModelContext<Self>) {
    ...
    pub fn indent(&mut self, _: &Indent, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
    ...
    pub fn outdent(&mut self, _: &Outdent, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
    ...
    pub fn delete_line(&mut self, _: &DeleteLine, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let selections = self.selections.all::<Point>(cx);
    ...
    pub fn join_lines(&mut self, _: &JoinLines, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
    ...
    pub fn revert_file(&mut self, _: &RevertFile, cx: &mut ModelContext<Self>) {
    ...
    pub fn reload_file(&mut self, _: &ReloadFile, cx: &mut ModelContext<Self>) {
        let Some(project) = self.project.clone() else {
            return;
        };
    ...
    pub fn reverse_lines(&mut self, _: &ReverseLines, cx: &mut ModelContext<Self>) {
        self.manipulate_lines(cx, |lines| lines.reverse())
    }

    pub fn shuffle_lines(&mut self, _: &ShuffleLines, cx: &mut ModelContext<Self>) {
        self.manipulate_lines(cx, |lines| lines.shuffle(&mut thread_rng()))
    }

    fn manipulate_lines<Fn>(&mut self, cx: &mut ModelContext<Self>, mut callback: Fn)
    where
    ...
    pub fn convert_to_title_case(&mut self, _: &ConvertToTitleCase, cx: &mut ModelContext<Self>) {
    ...
    pub fn convert_to_snake_case(&mut self, _: &ConvertToSnakeCase, cx: &mut ModelContext<Self>) {
        self.manipulate_text(cx, |text| text.to_case(Case::Snake))
    }

    pub fn convert_to_kebab_case(&mut self, _: &ConvertToKebabCase, cx: &mut ModelContext<Self>) {
    ...
    fn manipulate_text<Fn>(&mut self, cx: &mut ModelContext<Self>, mut callback: Fn)
    where
        Fn: FnMut(&str) -> String,
    {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
    ...
    pub fn duplicate_line(&mut self, upwards: bool, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let buffer = &display_map.buffer_snapshot;
        let selections = self.selections.all::<Point>(cx);
    ...
    pub fn duplicate_line_up(&mut self, _: &DuplicateLineUp, cx: &mut ModelContext<Self>) {
        self.duplicate_line(true, cx);
    }

    pub fn duplicate_line_down(&mut self, _: &DuplicateLineDown, cx: &mut ModelContext<Self>) {
        self.duplicate_line(false, cx);
    }

    pub fn move_line_up(&mut self, _: &MoveLineUp, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let buffer = self.buffer.read(cx).snapshot(cx);
    ...
    pub fn move_line_down(&mut self, _: &MoveLineDown, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let buffer = self.buffer.read(cx).snapshot(cx);
    ...
    pub fn transpose(&mut self, _: &Transpose, cx: &mut ModelContext<Self>) {
    ...
    pub fn rewrap(&mut self, _: &Rewrap, cx: &mut ModelContext<Self>) {
        self.rewrap_impl(IsVimMode::No, cx)
    }

    pub fn rewrap_impl(&mut self, is_vim_mode: IsVimMode, cx: &mut ModelContext<Self>) {
        let buffer = self.buffer.read(cx).snapshot(cx);
        let selections = self.selections.all::<Point>(cx);
        let mut selections = selections.iter().peekable();
    ...
    pub fn cut_common(&mut self, cx: &mut ModelContext<Self>) -> ClipboardItem {
        let mut text = String::new();
        let buffer = self.buffer.read(cx).snapshot(cx);
        let mut selections = self.selections.all::<Point>(cx);
    ...
    pub fn cut(&mut self, _: &Cut, cx: &mut ModelContext<Self>) {
        let item = self.cut_common(cx);
        cx.write_to_clipboard(item);
    }

    pub fn kill_ring_cut(&mut self, _: &KillRingCut, cx: &mut ModelContext<Self>) {
        self.change_selections(None, cx, |s| {
    ...
    pub fn kill_ring_yank(&mut self, _: &KillRingYank, cx: &mut ModelContext<Self>) {
    ...
    pub fn copy(&mut self, _: &Copy, cx: &mut ModelContext<Self>) {
        let selections = self.selections.all::<Point>(cx);
        let buffer = self.buffer.read(cx).read(cx);
        let mut text = String::new();
    ...
    pub fn paste(&mut self, _: &Paste, cx: &mut ModelContext<Self>) {
    ...
    pub fn undo(&mut self, _: &Undo, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
    ...
    pub fn redo(&mut self, _: &Redo, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
    ...
    pub fn group_until_transaction(&mut self, tx_id: TransactionId, cx: &mut ModelContext<Self>) {
        self.buffer
            .update(cx, |buffer, cx| buffer.group_until_transaction(tx_id, cx));
    }

    pub fn move_left(&mut self, _: &MoveLeft, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            let line_mode = s.line_mode;
            s.move_with(|map, selection| {
    ...
    pub fn select_left(&mut self, _: &SelectLeft, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_heads_with(|map, head, _| (movement::left(map, head), SelectionGoal::None));
        })
    }

    pub fn move_right(&mut self, _: &MoveRight, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            let line_mode = s.line_mode;
            s.move_with(|map, selection| {
                let cursor = if selection.is_empty() && !line_mode {
    ...
    pub fn select_right(&mut self, _: &SelectRight, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_heads_with(|map, head, _| (movement::right(map, head), SelectionGoal::None));
        })
    }

    pub fn move_up(&mut self, _: &MoveUp, cx: &mut ModelContext<Self>) {
    ...
    pub fn move_up_by_lines(&mut self, action: &MoveUpByLines, cx: &mut ModelContext<Self>) {
        if self.take_rename(true, cx).is_some() {
            return;
        }
    ...
    pub fn move_down_by_lines(&mut self, action: &MoveDownByLines, cx: &mut ModelContext<Self>) {
        if self.take_rename(true, cx).is_some() {
            return;
        }
    ...
    pub fn select_up_by_lines(&mut self, action: &SelectUpByLines, cx: &mut ModelContext<Self>) {
        let text_layout_details = &self.text_layout_details(cx);
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_heads_with(|map, head, goal| {
    ...
    pub fn select_page_up(&mut self, _: &SelectPageUp, cx: &mut ModelContext<Self>) {
    ...
    pub fn move_page_up(&mut self, action: &MovePageUp, cx: &mut ModelContext<Self>) {
        if self.take_rename(true, cx).is_some() {
            return;
        }
    ...
    pub fn select_up(&mut self, _: &SelectUp, cx: &mut ModelContext<Self>) {
        let text_layout_details = &self.text_layout_details(cx);
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_heads_with(|map, head, goal| {
    ...
    pub fn move_down(&mut self, _: &MoveDown, cx: &mut ModelContext<Self>) {
    ...
    pub fn select_page_down(&mut self, _: &SelectPageDown, cx: &mut ModelContext<Self>) {
        let Some(row_count) = self.visible_row_count() else {
            return;
        };
    ...
    pub fn move_page_down(&mut self, action: &MovePageDown, cx: &mut ModelContext<Self>) {
        if self.take_rename(true, cx).is_some() {
            return;
        }
    ...
    pub fn select_down(&mut self, _: &SelectDown, cx: &mut ModelContext<Self>) {
        let text_layout_details = &self.text_layout_details(cx);
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_heads_with(|map, head, goal| {
    ...
    pub fn context_menu_first(&mut self, _: &ContextMenuFirst, cx: &mut ModelContext<Self>) {
        if let Some(context_menu) = self.context_menu.write().as_mut() {
            context_menu.select_first(self.completion_provider.as_deref(), cx);
        }
    }

    pub fn context_menu_prev(&mut self, _: &ContextMenuPrev, cx: &mut ModelContext<Self>) {
        if let Some(context_menu) = self.context_menu.write().as_mut() {
            context_menu.select_prev(self.completion_provider.as_deref(), cx);
        }
    }

    pub fn context_menu_next(&mut self, _: &ContextMenuNext, cx: &mut ModelContext<Self>) {
        if let Some(context_menu) = self.context_menu.write().as_mut() {
            context_menu.select_next(self.completion_provider.as_deref(), cx);
        }
    }

    pub fn context_menu_last(&mut self, _: &ContextMenuLast, cx: &mut ModelContext<Self>) {
        if let Some(context_menu) = self.context_menu.write().as_mut() {
    ...
    pub fn move_to_next_word_end(&mut self, _: &MoveToNextWordEnd, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_cursors_with(|map, head, _| {
    ...
    pub fn move_to_end_of_line(&mut self, action: &MoveToEndOfLine, cx: &mut ModelContext<Self>) {
        self.change_selections(Some(Autoscroll::fit()), cx, |s| {
            s.move_cursors_with(|map, head, _| {
                (
    ...
    pub fn delete_to_end_of_line(&mut self, _: &DeleteToEndOfLine, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
    ...
    pub fn cut_to_end_of_line(&mut self, _: &CutToEndOfLine, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
            this.select_to_end_of_line(
                &SelectToEndOfLine {
                    stop_at_soft_wraps: false,
    ...
    pub fn move_to_beginning(&mut self, _: &MoveToBeginning, cx: &mut ModelContext<Self>) {
        if matches!(self.mode, EditorMode::SingleLine { .. }) {
            cx.propagate();
            return;
        }
    ...
    pub fn select_to_beginning(&mut self, _: &SelectToBeginning, cx: &mut ModelContext<Self>) {
    ...
    pub fn move_to_end(&mut self, _: &MoveToEnd, cx: &mut ModelContext<Self>) {
        if matches!(self.mode, EditorMode::SingleLine { .. }) {
            cx.propagate();
            return;
        }
    ...
    pub fn select_to_end(&mut self, _: &SelectToEnd, cx: &mut ModelContext<Self>) {
        let buffer = self.buffer.read(cx).snapshot(cx);
    ...
    pub fn select_all(&mut self, _: &SelectAll, cx: &mut ModelContext<Self>) {
    ...
    pub fn select_line(&mut self, _: &SelectLine, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let mut selections = self.selections.all::<Point>(cx);
    ...
    pub fn add_selection_above(&mut self, _: &AddSelectionAbove, cx: &mut ModelContext<Self>) {
        self.add_selection(true, cx);
    }

    pub fn add_selection_below(&mut self, _: &AddSelectionBelow, cx: &mut ModelContext<Self>) {
        self.add_selection(false, cx);
    }

    fn add_selection(&mut self, above: bool, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let mut selections = self.selections.all::<Point>(cx);
        let text_layout_details = self.text_layout_details(cx);
    ...
    pub fn select_next(&mut self, action: &SelectNext, cx: &mut ModelContext<Self>) -> Result<()> {
        self.push_to_selection_history();
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
    ...
    pub fn toggle_comments(&mut self, action: &ToggleComments, cx: &mut ModelContext<Self>) {
        if self.read_only(cx) {
            return;
        }
        let text_layout_details = &self.text_layout_details(cx);
    ...
    fn refresh_runnables(&mut self, cx: &mut ModelContext<Self>) -> Task<()> {
    ...
    pub fn undo_selection(&mut self, _: &UndoSelection, cx: &mut ModelContext<Self>) {
    ...
    pub fn redo_selection(&mut self, _: &RedoSelection, cx: &mut ModelContext<Self>) {
        self.end_selection(cx);
    ...
    pub fn expand_excerpts(&mut self, action: &ExpandExcerpts, cx: &mut ModelContext<Self>) {
    ...
    pub fn expand_excerpts_up(&mut self, action: &ExpandExcerptsUp, cx: &mut ModelContext<Self>) {
    ...
    fn go_to_diagnostic(&mut self, _: &GoToDiagnostic, cx: &mut ModelContext<Self>) {
        self.go_to_diagnostic_impl(Direction::Next, cx)
    }

    fn go_to_prev_diagnostic(&mut self, _: &GoToPrevDiagnostic, cx: &mut ModelContext<Self>) {
        self.go_to_diagnostic_impl(Direction::Prev, cx)
    }

    pub fn go_to_diagnostic_impl(&mut self, direction: Direction, cx: &mut ModelContext<Self>) {
        let buffer = self.buffer.read(cx).snapshot(cx);
    ...
    fn go_to_next_hunk(&mut self, _: &GoToHunk, cx: &mut ModelContext<Self>) {
        let snapshot = self
            .display_map
            .update(cx, |display_map, cx| display_map.snapshot(cx));
    ...
    fn go_to_prev_hunk(&mut self, _: &GoToPrevHunk, cx: &mut ModelContext<Self>) {
        let snapshot = self
            .display_map
            .update(cx, |display_map, cx| display_map.snapshot(cx));
        let selection = self.selections.newest::<Point>(cx);
    ...
    pub fn open_url(&mut self, _: &OpenUrl, cx: &mut ModelContext<Self>) {
        let position = self.selections.newest_anchor().head();
    ...
    pub fn open_file(&mut self, _: &OpenFile, cx: &mut ModelContext<Self>) {
    ...
    pub fn rename(&mut self, _: &Rename, cx: &mut ModelContext<Self>) -> Option<Task<Result<()>>> {
        use language::ToOffset as _;
    ...
    fn format(&mut self, _: &Format, cx: &mut ModelContext<Self>) -> Option<Task<Result<()>>> {
    ...
    fn restart_language_server(&mut self, _: &RestartLanguageServer, cx: &mut ModelContext<Self>) {
    ...
    fn show_character_palette(&mut self, _: &ShowCharacterPalette, cx: &mut ModelContext<Self>) {
        cx.show_character_palette();
    }

    fn refresh_active_diagnostics(&mut self, cx: &mut ModelContext<Editor>) {
    ...
    fn dismiss_diagnostics(&mut self, cx: &mut ModelContext<Self>) {
    ...
    pub fn transact(
        &mut self,
        cx: &mut ModelContext<Self>,
        update: impl FnOnce(&mut Self, &mut ModelContext<Self>),
    ...
    fn start_transaction_at(&mut self, now: Instant, cx: &mut ModelContext<Self>) {
        self.end_selection(cx);
    ...
    pub fn toggle_fold(&mut self, _: &actions::ToggleFold, cx: &mut ModelContext<Self>) {
        let selection = self.selections.newest::<Point>(cx);
    ...
    pub fn fold(&mut self, _: &actions::Fold, cx: &mut ModelContext<Self>) {
    ...
    fn fold_at_level(&mut self, fold_at: &FoldAtLevel, cx: &mut ModelContext<Self>) {
    ...
    pub fn fold_all(&mut self, _: &actions::FoldAll, cx: &mut ModelContext<Self>) {
    ...
    pub fn fold_recursive(&mut self, _: &actions::FoldRecursive, cx: &mut ModelContext<Self>) {
        let mut to_fold = Vec::new();
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let selections = self.selections.all_adjusted(cx);
    ...
    pub fn fold_at(&mut self, fold_at: &FoldAt, cx: &mut ModelContext<Self>) {
    ...
    pub fn unfold_lines(&mut self, _: &UnfoldLines, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let buffer = &display_map.buffer_snapshot;
        let selections = self.selections.all::<Point>(cx);
    ...
    pub fn unfold_recursive(&mut self, _: &UnfoldRecursive, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        let selections = self.selections.all::<Point>(cx);
        let ranges = selections
            .iter()
    ...
    pub fn unfold_at(&mut self, unfold_at: &UnfoldAt, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
    ...
    pub fn unfold_all(&mut self, _: &actions::UnfoldAll, cx: &mut ModelContext<Self>) {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
        self.unfold_ranges(&[0..display_map.buffer_snapshot.len()], true, true, cx);
    }

    pub fn fold_selected_ranges(&mut self, _: &FoldSelectedRanges, cx: &mut ModelContext<Self>) {
        let selections = self.selections.all::<Point>(cx);
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
    ...
    pub fn set_gutter_hovered(&mut self, hovered: bool, cx: &mut ModelContext<Self>) {
    ...
    pub fn set_text(&mut self, text: impl Into<Arc<str>>, cx: &mut ModelContext<Self>) {
        self.transact(cx, |this, cx| {
            this.buffer
                .read(cx)
                .as_singleton()
    ...
    pub(crate) fn set_style(&mut self, style: EditorStyle, cx: &mut ModelContext<Self>) {
    ...
    pub fn toggle_soft_wrap(&mut self, _: &ToggleSoftWrap, cx: &mut ModelContext<Self>) {
    ...
    pub fn toggle_tab_bar(&mut self, _: &ToggleTabBar, cx: &mut ModelContext<Self>) {
        let Some(workspace) = self.workspace() else {
            return;
        };
    ...
    pub fn toggle_indent_guides(&mut self, _: &ToggleIndentGuides, cx: &mut ModelContext<Self>) {
    ...
    pub fn toggle_line_numbers(&mut self, _: &ToggleLineNumbers, cx: &mut ModelContext<Self>) {
    ...
    pub fn set_show_gutter(&mut self, show_gutter: bool, cx: &mut ModelContext<Self>) {
        self.show_gutter = show_gutter;
        cx.notify();
    }

    pub fn set_show_line_numbers(&mut self, show_line_numbers: bool, cx: &mut ModelContext<Self>) {
    ...
    pub fn set_show_code_actions(&mut self, show_code_actions: bool, cx: &mut ModelContext<Self>) {
        self.show_code_actions = Some(show_code_actions);
        cx.notify();
    }

    pub fn set_show_runnables(&mut self, show_runnables: bool, cx: &mut ModelContext<Self>) {
        self.show_runnables = Some(show_runnables);
        cx.notify();
    }

    pub fn set_masked(&mut self, masked: bool, cx: &mut ModelContext<Self>) {
    ...
    pub fn set_show_wrap_guides(&mut self, show_wrap_guides: bool, cx: &mut ModelContext<Self>) {
    ...
    pub fn reveal_in_finder(&mut self, _: &RevealInFileManager, cx: &mut ModelContext<Self>) {
        if let Some(target) = self.target_file(cx) {
            cx.reveal_path(&target.abs_path(cx));
        }
    }

    pub fn copy_path(&mut self, _: &CopyPath, cx: &mut ModelContext<Self>) {
    ...
    pub fn copy_relative_path(&mut self, _: &CopyRelativePath, cx: &mut ModelContext<Self>) {
        if let Some(file) = self.target_file(cx) {
    ...
    pub fn toggle_selection_menu(&mut self, _: &ToggleSelectionMenu, cx: &mut ModelContext<Self>) {
    ...
    fn start_git_blame(&mut self, user_triggered: bool, cx: &mut ModelContext<Self>) {
    ...
    fn start_git_blame_inline(&mut self, user_triggered: bool, cx: &mut ModelContext<Self>) {
    ...
    fn get_permalink_to_line(&mut self, cx: &mut ModelContext<Self>) -> Task<Result<url::Url>> {
    ...
    pub fn copy_permalink_to_line(&mut self, _: &CopyPermalinkToLine, cx: &mut ModelContext<Self>) {
    ...
    pub fn copy_file_location(&mut self, _: &CopyFileLocation, cx: &mut ModelContext<Self>) {
    ...
    pub fn open_permalink_to_line(&mut self, _: &OpenPermalinkToLine, cx: &mut ModelContext<Self>) {
        let permalink_task = self.get_permalink_to_line(cx);
        let workspace = self.workspace();
    ...
    pub fn clear_search_within_ranges(&mut self, cx: &mut ModelContext<Self>) {
    ...
    pub fn clear_highlights<T: 'static>(&mut self, cx: &mut ModelContext<Self>) {
        let cleared = self
            .display_map
    ...
    fn on_buffer_changed(&mut self, _: Model<MultiBuffer>, cx: &mut ModelContext<Self>) {
        cx.notify();
    }
    ...
    fn on_display_map_changed(&mut self, _: Model<DisplayMap>, cx: &mut ModelContext<Self>) {
        cx.notify();
    }
    ...
    pub fn open_excerpts_in_split(&mut self, _: &OpenExcerptsSplit, cx: &mut ModelContext<Self>) {
        self.open_excerpts_common(None, true, cx)
    }

    pub fn open_excerpts(&mut self, _: &OpenExcerpts, cx: &mut ModelContext<Self>) {
    ...
    fn copy_highlight_json(&mut self, _: &CopyHighlightJson, cx: &mut ModelContext<Self>) {
        let Some(buffer) = self.buffer.read(cx).as_singleton() else {
            return;
        };
    ...
    fn handle_focus(&mut self, cx: &mut ModelContext<Self>) {
    ...
    fn handle_focus_in(&mut self, cx: &mut ModelContext<Self>) {
        cx.emit(EditorEvent::FocusedIn)
    }

    fn handle_focus_out(&mut self, event: FocusOutEvent, _cx: &mut ModelContext<Self>) {
        if event.blurred != self.focus_handle {
            self.last_focused_descendant = Some(event.blurred);
        }
    }

    pub fn handle_blur(&mut self, cx: &mut ModelContext<Self>) {
...
impl Render for Editor {
    fn render<'a>(&mut self, cx: &mut ModelContext<'a, Self>) -> impl IntoElement {
...
impl ViewInputHandler for Editor {
    ...
    fn marked_text_range(&self, cx: &mut ModelContext<Self>) -> Option<Range<usize>> {
        let snapshot = self.buffer.read(cx).read(cx);
        let range = self.text_highlights::<InputComposition>(cx)?.1.first()?;
        Some(range.start.to_offset_utf16(&snapshot).0..range.end.to_offset_utf16(&snapshot).0)
    }

    fn unmark_text(&mut self, cx: &mut ModelContext<Self>) {
