...
use editor::Editor;
use gpui::{prelude::*, Entity, ModelContext, View, WeakView, WindowContext};
...
pub fn assign_kernelspec(
    ...
    let session = cx
        .new_model(|cx| Session::new(weak_editor.clone(), fs, telemetry, kernel_specification, cx));
...
pub fn run(
    ...
    for runnable_range in runnable_ranges {
        ...
        let session = if let Some(session) = store.read(cx).get_session(editor.entity_id()).cloned()
        {
            ...
            let session = cx
                .new_model(|cx| Session::new(weak_editor, fs, telemetry, kernel_specification, cx));
...
pub fn setup_editor_session_actions(editor: &mut Editor, editor_handle: WeakModel<Editor>) {
    ...
    editor
        .register_action({
            ...
            move |_: &Shutdown, window, cx: &mut AppContext| {
                if !JupyterSettings::enabled(cx) {
                    return;
                }
