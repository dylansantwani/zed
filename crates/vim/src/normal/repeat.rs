...
use editor::Editor;
use gpui::{actions, Action, ModelContext, WindowContext};
use util::ResultExt;
...
impl Vim {
    pub(crate) fn record_register(&mut self, register: char, cx: &mut ModelContext<Self>) {
        let globals = Vim::globals(cx);
    ...
    pub(crate) fn replay_register(&mut self, mut register: char, cx: &mut ModelContext<Self>) {
