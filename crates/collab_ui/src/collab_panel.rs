...
use gpui::{
    ...
    ListOffset, ListState, Model, ModelContext, ModelContext, MouseDownEvent, ParentElement,
    Pixels, Point, PromptLevel, Render, SharedString, Styled, Subscription, Task, TextStyle, View,
...
impl CollabPanel {
    pub fn new(workspace: &mut Workspace, cx: &mut ModelContext<Workspace>) -> Model<Self> {
        cx.new_model(|cx| {
            ...
            let channel_name_editor = cx.new_model(Editor::single_line);
    ...
    fn update_entries(&mut self, select_same_item: bool, cx: &mut ModelContext<Self>) {
        ...
        let cx: &mut ModelContext<Self> = cx; // todo! what's the deal
        cx.notify();
    }
        ...
        ListItem::new(SharedString::from(user.github_login.clone()))
            ...
            .end_slot(if is_pending {
                ...
                IconButton::new("leave-call", IconName::Exit)
                    ...
                    .on_click(move |_, cx| Self::leave_call(window, cx))
    ...
    fn take_editing_state(&mut self, cx: &mut ModelContext<Self>) -> bool {
                ...
                context_menu = context_menu.entry(
                    ...
                    window.handler_for(&this, move |_, window, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("Failed to grant write access", window, cx, |e, _window, _cx| {
                                match e.error_code() {
    ...
    fn reset_filter_editor_text(&mut self, cx: &mut ModelContext<Self>) -> bool {
    ...
    fn select_next(&mut self, _: &SelectNext, cx: &mut ModelContext<Self>) {
    ...
    fn select_prev(&mut self, _: &SelectPrev, cx: &mut ModelContext<Self>) {
    ...
    fn insert_space(&mut self, _: &InsertSpace, cx: &mut ModelContext<Self>) {
    ...
    fn confirm_channel_edit(&mut self, cx: &mut ModelContext<CollabPanel>) -> bool {
    ...
    fn toggle_section_expanded(&mut self, section: Section, cx: &mut ModelContext<Self>) {
    ...
    fn expand_selected_channel(&mut self, _: &ExpandSelectedChannel, cx: &mut ModelContext<Self>) {
    ...
    fn toggle_channel_collapsed(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
    ...
    fn toggle_contact_finder(&mut self, cx: &mut ModelContext<Self>) {
        if let Some(workspace) = self.workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
    ...
    fn new_root_channel(&mut self, cx: &mut ModelContext<Self>) {
    ...
    fn new_subchannel(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
    ...
    fn manage_members(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        self.show_channel_modal(channel_id, channel_modal::Mode::ManageMembers, cx);
    }

    fn remove_selected_channel(&mut self, _: &Remove, cx: &mut ModelContext<Self>) {
        if let Some(channel) = self.selected_channel() {
            self.remove_channel(channel.id, cx)
        }
    }

    fn rename_selected_channel(&mut self, _: &SecondaryConfirm, cx: &mut ModelContext<Self>) {
        if let Some(channel) = self.selected_channel() {
            self.rename_channel(channel.id, cx);
        }
    }

    fn rename_channel(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        let channel_store = self.channel_store.read(cx);
    ...
    fn set_channel_visibility(
        ...
        self.channel_store
            ...
            .detach_and_prompt_err("Failed to set channel visibility", window, cx, |e, _window, _cx| match e.error_code() {
    ...
    fn start_move_channel(&mut self, channel_id: ChannelId, _cx: &mut ModelContext<Self>) {
        self.channel_clipboard = Some(ChannelMoveClipboard { channel_id });
    }

    fn start_move_selected_channel(&mut self, _: &StartMoveChannel, cx: &mut ModelContext<Self>) {
        if let Some(channel) = self.selected_channel() {
    ...
    fn move_channel_on_clipboard(
        ...
        cx: &mut ModelContext<CollabPanel>,
    ) {
    ...
    fn move_channel(&self, channel_id: ChannelId, to: ChannelId, cx: &mut ModelContext<Self>) {
        self.channel_store
            .update(cx, |channel_store, cx| {
            ...
            .detach_and_prompt_err(
                ...
                window,
                cx,
                |e, _window, _| match e.error_code() {
                    ErrorCode::BadPublicNesting => {
    ...
    fn open_channel_notes(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        if let Some(workspace) = self.workspace.upgrade() {
    ...
    fn leave_channel(&self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
    ...
    fn remove_channel(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        let channel_store = self.channel_store.clone();
    ...
    fn remove_contact(&mut self, user_id: u64, github_login: &str, cx: &mut ModelContext<Self>) {
        let user_store = self.user_store.clone();
        let prompt_message = format!(
    ...
    fn call(&mut self, recipient_user_id: u64, cx: &mut ModelContext<Self>) {
        ActiveCall::global(cx)
            .update(cx, |call, cx| {
    ...
    fn join_channel(&self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
    ...
    fn join_channel_chat(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        let Some(workspace) = self.workspace.upgrade() else {
            return;
        };
        cx.window_context().defer(move |cx| {
    ...
    fn copy_channel_link(&mut self, channel_id: ChannelId, cx: &mut ModelContext<Self>) {
        let channel_store = self.channel_store.read(cx);
    ...
    fn render_signed_out(&mut self, cx: &mut ModelContext<Self>) -> Div {
    ...
    fn render_list_entry(&mut self, ix: usize, cx: &mut ModelContext<Self>) -> AnyElement {
    ...
    fn render_signed_in(&mut self, cx: &mut ModelContext<Self>) -> Div {
        ...
        div()
            ...
            .when(!channel.is_root_channel(), |el| {
                el.on_drag(channel.clone(), move |channel, _, cx| {
                    cx.new_model(|_| DraggedChannelView {
                        channel: channel.clone(),
                        width,
                    })
                })
...
impl Panel for CollabPanel {
    ...
    fn set_position(&mut self, position: DockPosition, cx: &mut ModelContext<Self>) {
